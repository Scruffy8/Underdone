...
function EFFECT:Think( )
	...
	if self.EffectDelay < CurTime() then
		...
		for i=0, 8 do	
			local Smoke = self.Emitter:Add( "sprites/physg_glow1",  self.Player:GetPos() + vecRand() * 30 )
			if (Smoke) then
				Smoke:SetVelocity( Vector( 0, 0, 60 ) + vecRand() * 60 )
				Smoke:SetDieTime( .5)
		...
		local Angle = Vector( 0, 0, 1 ):Angle()
		for i=0, 3 do	
			local Light = self.Emitter:Add( "sprites/physg_glow2",  self.Player:GetPos() + Vector( 0, 0, 20 ) + vecRand() * 20 )
			if (Light) then
				local speed = vecRand() * 200 
				speed.z = 0
