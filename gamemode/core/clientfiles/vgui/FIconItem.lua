...
function PANEL:SetItem(tblItemTable, intAmount, strUseCommand, intCost)
	...
	if strUseCommand == "use" && tblItemTable.Dropable then
		self.DoDropItem = function( dropall )
			if dropall then
				...
				self:RunPromtAmount(tblItemTable, intAmount, langopt[lang_cur].howmanydrop, "UD_DropItem")
			end
		end
	end
	if strUseCommand == "use" && tblItemTable.Giveable then
		self.DoGiveItem = function(plyGivePlayer)
			if tblItemTable.Stackable or intAmount >= 5 then 
				GAMEMODE:DisplayPromt("number", langopt[lang_cur].howmanygive, function(itemamount)
	...
	if strUseCommand == "sell" then
		self.DoUseItem = function(intAmountToSell)
			self:RunPromtAmount(tblItemTable, intAmount, langopt[lang_cur].howmanysell, "UD_SellItem", intAmountToSell)
		end
	end
	if strUseCommand == "deposit" then
		self.DoUseItem = function(intAmountToDipostite)
			self:RunPromtAmount(tblItemTable, intAmount, langopt[lang_cur].howmanydeposit, "UD_DipostiteItem", intAmountToDipostite)
		end
	end
	if strUseCommand == "withdraw" then
		self.DoUseItem = function(intAmountToWithdraw)
			self:RunPromtAmount(tblItemTable, intAmount, langopt[lang_cur].howmanywithdraw, "UD_WithdrawItem", intAmountToWithdraw)
		end
	end
	...
	if strUseCommand == "buy" && intCost > 0 then
		if tblItemTable.Currency == "money" then
			strTooltip = Format(langopt[lang_cur].itm_buyfor_d, strTooltip, intCost)
		else
			local tblCurTable = ItemTable( tblItemTable.Currency )
			if tblCurTable then
				strTooltip = Format(langopt[lang_cur].itm_buyfor, strTooltip, intCost, tblCurTable.PrintName)
			end
		end
	end
	if strUseCommand == "sell" && intCost > 0 then
		if tblItemTable.Currency == "money" then
			strTooltip = Format(langopt[lang_cur].itm_sellfor_d, strTooltip, intCost)
		else
			local tblCurTable = ItemTable( tblItemTable.Currency )
			if tblCurTable then
				strTooltip = Format(langopt[lang_cur].itm_sellfor, strTooltip, intCost, tblCurTable.PrintName)
			end
		end
	end
	...
	local menuFunc = function()
		...
		if strUseCommand == "use" && tblItemTable.Use && self.DoUseItem then GAMEMODE.ActiveMenu:AddOption(langopt[lang_cur].use, function() self.DoUseItem() end) end
		if strUseCommand == "buy" && self.DoUseItem then GAMEMODE.ActiveMenu:AddOption(langopt[lang_cur].buy, function() self.DoUseItem() end) end
		if strUseCommand == "sell" && intCost > 0 && self.DoUseItem then GAMEMODE.ActiveMenu:AddOption(langopt[lang_cur].sell, function() self.DoUseItem() end) end
		if strUseCommand == "sell" && intCost > 0 && intAmount > 1 then GAMEMODE.ActiveMenu:AddOption(langopt[lang_cur].sellall, function() self.DoUseItem(intAmount) end) end
		if strUseCommand == "deposit" && self.DoUseItem then GAMEMODE.ActiveMenu:AddOption(langopt[lang_cur].deposit, function() self.DoUseItem() end) end
		if strUseCommand == "withdraw" && self.DoUseItem then GAMEMODE.ActiveMenu:AddOption(langopt[lang_cur].withdraw, function() self.DoUseItem() end) end
		if strUseCommand == "use" && tblItemTable.Dropable then GAMEMODE.ActiveMenu:AddOption(langopt[lang_cur].drop, function() self.DoDropItem() end) end
