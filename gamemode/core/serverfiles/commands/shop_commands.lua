...
function Player:BuyItem(strItem, intAmount)
	...
	if tblNPCTable && tblShopTable && tblShopTable.Inventory[strItem] then
		...
		if self:HasItem(tblItemTable.Currency, intPrice) && self:AddItem(strItem, intAmount) then
			self:EmitSound( "blacktea/ui_shop_buy.wav" )
			self:RemoveItem(tblItemTable.Currency, intPrice)
		end
	end
end
...
function Player:SellItem(strItem, intAmount)
	...
	if tblNPCTable && tblNPCTable.Shop && self:HasItem(strItem, intAmount) then
		...
		if tblItemTable.SellPrice > 0 && self:RemoveItem(strItem, intAmount) then
			...
			self:AddItem(tblItemTable.Currency, tblItemTable.SellPrice * intAmount)
		end
	end
end
